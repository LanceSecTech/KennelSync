...
export default function MyDogs() {
  ...
  const createDog = trpc.dog.create.useMutation({
    onSuccess: () => {
      ...
      setShowAdd(false);
      setForm({ name: "", breed: "", age: "", sex: "", weight: "", isSpayedNeutered: false });
...
function DogCard({ dog, onClick }: { dog: any; onClick: () => void }) {
  ...
  const missingRequired = (activeKennelId && requiredVaccines) ? requiredVaccines.filter(rv =>
    !vaccinations?.some(v =>
      v.vaccineName.toLowerCase().trim() === rv.vaccineName.toLowerCase().trim() && v.status !== 'expired' && v.status !== 'missing'
    )
  ) : [];
  const hasExpiredVaccines = vaccinations?.some((v: any) => v.status === "expired") || false;
                    ...
                    <TooltipContent side="bottom">
                      <p className="text-xs">
                        {missingRequired.length > 0
                          ? `Missing required: ${missingRequired.map(rv => rv.vaccineName).join(", ")}`
