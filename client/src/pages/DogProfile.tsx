...
import {
  ArrowLeft, Shield, Utensils, Pill, Heart, Stethoscope, Phone, Plus, Trash2,
...
export default function DogProfile() {
  ...
  const canSeeBadges = user?.role === "owner" || user?.role === "employee";
  ...
  const { data: badgeCatalog } = trpc.dogBadge.listByKennel.useQuery(
    { kennelId: activeKennelId! },
    { enabled: !!activeKennelId && canSeeBadges },
  );
  const { data: badgeAssignments } = trpc.dogBadge.assignedForDogs.useQuery(
    { kennelId: activeKennelId!, dogIds: [dogId] },
    { enabled: !!activeKennelId && !!dogId && canSeeBadges },
  );
  const assignBadges = trpc.dogBadge.assignToDog.useMutation({
    onSuccess: () => {
      utils.dogBadge.assignedForDogs.invalidate();
      utils.dog.getById.invalidate({ id: dogId });
      toast.success("Dog badges updated");
    },
    onError: (e) => toast.error(e.message || "Could not update badges"),
  });
    ...
    <div className="p-4 space-y-4 max-w-2xl mx-auto">
            ...
            <div className="flex-1">
              <div className="grid grid-cols-2 gap-y-1.5 text-xs">
                ...
                <div>
                  ...
                  <span className="font-medium">{dog.birthday ? formatDate(dog.birthday) : "—"}</span>
                </div>
                <div>
              ...
              {activeKennelId && canSeeBadges && (
                <div className="mt-2">
                  <DogBadgesInline
                    badgeKeys={(badgeAssignments as any)?.[String(dogId)] || []}
                    badgeByKey={new Map(((badgeCatalog || []) as any[]).map((b: any) => [String(b.key || "").toLowerCase(), b]))}
                    max={8}
                  />
                </div>
              )}
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {activeKennelId && canSeeBadges && (
                  <Button size="sm" variant="outline" className="gap-1 h-7 text-xs mt-2.5" onClick={() => setBadgeDialogOpen(true)}>
                    Badges
                  </Button>
                )}
              </div>
      ...
      <Dialog open={badgeDialogOpen} onOpenChange={setBadgeDialogOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle>Assign Dog Badges</DialogTitle></DialogHeader>
          <div className="space-y-2 max-h-[50vh] overflow-y-auto">
            {(badgeCatalog || []).map((b: any) => {
              const assigned = ((badgeAssignments as any)?.[String(dogId)] || []).includes(String(b.key || "").toLowerCase());
              return (
                <label key={b.key} className="flex items-start gap-2 rounded border p-2">
                  <Checkbox
                    checked={assigned}
                    onCheckedChange={(checked) => {
                      const current = new Set<string>(((badgeAssignments as any)?.[String(dogId)] || []).map((x: string) => String(x).toLowerCase()));
                      if (checked) current.add(String(b.key || "").toLowerCase());
                      else current.delete(String(b.key || "").toLowerCase());
                      assignBadges.mutate({ kennelId: activeKennelId!, dogId, badgeKeys: Array.from(current) });
                    }}
                  />
                  <div>
      ...
      <Tabs defaultValue="vaccinations" className="w-full">
        ...
        <TabsContent value="vaccinations" className="space-y-3 mt-3">
              ...
              <DialogContent className="max-w-sm">
                ...
                <div className="space-y-3 mt-2">
                          ...
                          <Select value={vaxForm.vaccineName} onValueChange={v => {
                            ...
                            <SelectContent>
                              {requiredVaccines
                                ...
                                .map(({ rv, label }) => (
                                  <SelectItem key={rv.id} value={label}>{label}</SelectItem>
                                ))}
                  ...
                  <Button onClick={() => {
                    ...
                    if (!vaxForm.expirationDate.trim()) { toast.error("Expiration date required"); return; }
                    createVax.mutate({
                      dogId,
                      ...(user?.role === "owner" && activeKennelId ? { kennelId: activeKennelId } : {}),
          ...
          {(vaccinations ?? []).filter(v => v != null && v.id != null).map(vax => (
            <VaccinationCard
              key={vax.id}
              vax={vax}
...
function EditInfoDialog({ dog, dogId, onSave, isPending }: {
  ...
  const [form, setForm] = useState({
    name: "", breed: "", age: "", weight: "", birthday: "", sex: "" as string, isSpayedNeutered: false,
  });
  ...
  useEffect(() => {
    if (dog && open) {
      setForm({
        ...
        weight: dog.weight || "",
        birthday: dog.birthday ? toDateString(dog.birthday) : "",
        sex: dog.sex || "",
...
function VaccinationCard({ vax, onDelete, onUploadCert, onRemoveCert }: {
  ...
  const status = vaxStatusSafe(vax);
  const name = vaxName(vax) || "Vaccination";
  const given = vaxDateGiven(vax);
  const exp = vaxExpires(vax);
  const doc = vaxDocUrl(vax);
  return (
    <Card className="border-0 shadow-sm bg-white">
      <CardContent className="p-3 space-y-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            {status === "current" && <CheckCircle2 className="h-4 w-4 text-green-600" />}
            {status === "expiring_soon" && <Clock className="h-4 w-4 text-amber-500" />}
            {status === "expired" && <AlertCircle className="h-4 w-4 text-destructive" />}
            {status === "missing" && <AlertCircle className="h-4 w-4 text-muted-foreground" />}
            {!["current", "expiring_soon", "expired", "missing"].includes(status) && (
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground">
                {[
                  given ? `Given: ${formatDate(given)}` : null,
                  exp ? `Expires: ${formatDate(exp)}` : null,
                ]
                  .filter(Boolean)
                  .join(" · ") || "No expiration set"}
              </p>
            </div>
          </div>
          <button type="button" onClick={onDelete} className="p-1.5 rounded hover:bg-muted">
        ...
        <div className="pl-6">
          {doc ? (
            <div className="flex items-center gap-2 p-2 rounded-lg bg-primary/5 border border-primary/10">
              ...
              <a href={doc} target="_blank" rel="noopener noreferrer" className="text-xs text-primary font-medium truncate hover:underline flex-1">
                View Certificate
              </a>
              <FileUpload onUpload={onUploadCert} currentUrl={doc} onRemove={onRemoveCert} accept="image/*,.pdf" folder="vaccination-certs" variant="document" label="Replace" maxSizeMB={10} />
            </div>
          ) : (
